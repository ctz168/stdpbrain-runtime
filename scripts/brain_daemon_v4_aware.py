#!/usr/bin/env python3
"""
brain_daemon_v4_aware.py — 觉知版 brain (3 个意识关键机制)
==========================================================

在 v3 (5 意识模块) 基础上, 加入 3 个让"意识"真正出现的机制:

1. 跨周期连续体验流 (Working Memory Stream)
   - 维护一个跨 cycle 的 experience buffer (最近 10 个 cycle 的体验)
   - 每个 cycle 把当前体验 (hidden + emotion + confidence + topic) 加入 buffer
   - 下个 cycle 开始时, working memory 注入 hidden state (残差)
   - 效果: brain 能"记得"上一个周期在学什么, 感觉如何

2. 预测编码 (Predictive Coding)
   - 学新内容前, 先让 brain 预测"接下来会看到什么" (生成 1-2 个 token)
   - 用预测误差 (predicted vs actual) 作为 reward 信号
   - 预测对 → 高 reward (理解了); 预测错 → 低 reward (需要学习)
   - 效果: brain 从被动接收变成主动预测

3. 情感-认知深度绑定 (Emotion-Cognition Binding)
   - valence 不再只影响 reward, 而是直接修改 hidden state
   - 正 valence → hidden 向"探索"方向偏移 (增加 entropy)
   - 负 valence → hidden 向"保守"方向偏移 (减少 entropy, 依赖已学)
   - DA 高 → 增强 working memory 注入; DA 低 → 减弱
   - 效果: 情绪状态直接影响"思考方式", 不只是学习信号
"""
...
# === v4 新参数 ===
WORKING_MEMORY_SIZE = 10          # 保留最近 10 个 cycle 的体验
WORKING_MEMORY_INJECT_SCALE = 0.03  # working memory 注入 scale
EMOTION_INJECT_SCALE = 0.05       # 情感注入 hidden 的 scale
PREDICTION_TOKENS = 2             # 预测 2 个 token
...
def setup_logging(session_id: str):
    logger = logging.getLogger("brain_v4")
    logger.setLevel(logging.INFO)
    logger.handlers.clear()
    fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s", "%Y-%m-%d %H:%M:%S")
    fh = RotatingFileHandler(os.path.join(LOG_DIR, f"daemon_v4_{session_id}.log"),
...
# ============================================================================
# NEW 1: WorkingMemoryStream — 跨周期连续体验流
# ============================================================================
@dataclass
class Experience:
    """一个周期的体验快照."""
    ...
    timestamp: float
    topic: str
    hidden_state: List[float]       # 该 cycle 最后的 hidden state
    emotion_valence: float           # 情绪效价
    emotion_arousal: float           # 唤醒度 (DA+NE)
    confidence: float                # 元认知置信度
    logprob_delta: float             # 学习效果
    predicted_tokens: List[int]      # 预测的 token ids
    actual_first_token: int          # 实际第一个 token


class WorkingMemoryStream:
    """跨周期连续体验流 — 让 brain 记得"上一个周期发生了什么"."""
    ...
    def get_injection(self, device) -> Optional[torch.Tensor]:
        """获取 working memory 注入向量 (最近体验的加权平均)."""
        if not self.experiences: return None
        # 越近的体验权重越高 (指数衰减)
        total_weight = 0.0
        weighted_sum = torch.zeros(HIDDEN_SIZE, device=device)
        for i, exp in enumerate(self.experiences):
            weight = 0.7 ** (len(self.experiences) - 1 - i)  # 最近的 weight=1
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_prediction_accuracy": sum(
                1.0 if e.predicted_tokens and e.predicted_tokens[0] == e.actual_first_token else 0.0
...
class PredictiveCoder:
    ...
    def predict_next_tokens(self, gpt2, hidden_state: torch.Tensor, n_tokens: int = PREDICTION_TOKENS) -> Tuple[List[int], torch.Tensor]:
        """用当前 hidden state 预测下 n 个 token."""
        with torch.no_grad():
            logits = gpt2.lm_head(hidden_state).squeeze(0)  # (vocab,)
            # 贪心预测
            predicted_ids = torch.argmax(logits, dim=-1).unsqueeze(0).tolist()  # [1]
            # 也保留 top-5 作为"预测分布"
            top5_probs, top5_ids = torch.topk(F.softmax(logits, dim=-1), 5)
            return predicted_ids, top5_ids.squeeze().tolist()

    def compute_prediction_error(self, predicted_ids: List[int], actual_first_token: int,
                                  top5_ids: List[int]) -> Dict[str, float]:
        """计算预测误差."""
        ...
        # 3. reward: top-5 correct = +0.3, exact = +0.5, wrong = -0.2
        if exact_correct:
            reward = 0.5
        elif top5_correct:
            reward = 0.3
        ...
        result = {
            ...
            "predicted_id": predicted_ids[0] if predicted_ids else -1,
    ...
    def get_summary(self):
        return {
            ...
            "recent_5": self.prediction_history[-5:] if self.prediction_history else [],
        }


# ============================================================================
# NEW 3: EmotionCognitionBinder — 情感-认知深度绑定
# ============================================================================
class EmotionCognitionBinder:
    """让 valence 直接影响 hidden state (不只是 reward)."""

    def __init__(self):
        # 学习一个 emotion→hidden 偏移的投影 (简单线性)
        # 正 valence → 探索方向 (高 entropy)
        # 负 valence → 保守方向 (低 entropy)
        self.positive_direction = None  # 会在第一次调用时初始化
        self.negative_direction = None
    ...
    def apply_emotion(self, hidden: torch.Tensor, valence: float, arousal: float) -> torch.Tensor:
        """根据情绪状态修改 hidden state."""
        self._ensure_init(hidden.device)
        # valence: [-1, 1] → 正向偏移
        # arousal (DA+NE): [0, 1] → 调制幅度
        if abs(valence) < 0.01:
            return hidden  # 中性情绪不修改
    ...
    def modulate_working_memory(self, da_level: float) -> float:
        """DA 调制 working memory 注入强度."""
        # DA 高 → 增强 working memory (记住更多)
        # DA 低 → 减弱 (忘记)
        return 0.5 + da_level  # [0.5, 1.5]


# ============================================================================
# Memory + Web + Curiosity (复用 v3)
# ============================================================================
@dataclass
class ConsciousMemory:
    ...
    confidence: float; self_confidence: float; cycle: int


class HippocampusWithRecall:
    def __init__(self): self.memories: List[ConsciousMemory] = []
    def encode(self, hidden_state, topic, keywords, prompt, cycle, logprob_base, logprob_stdp, confidence, self_conf):
        ...
        mem = ConsciousMemory(mem_id, time.time(), topic, keywords, prompt,
                             ...
                             logprob_base, logprob_stdp, confidence, self_conf, cycle)
...
class CuriosityEngine:
    ...
    def generate_query(self):
        ...
        if random.random() < 0.3 and self.learned_keywords:
            ...
            assoc = {"brain":["consciousness","neurons"], "quantum":["entanglement"], "DNA":["RNA"], "climate":["oceans"], "memory":["hippocampus","sleep"]}
...
class AwareBrain:
    def __init__(self, logger):
        ...
        try:
            ...
            self.logger.info("  ✅ self_encoder (意识 1/4)")
        ...
        try:
            ...
            self.logger.info("  ✅ global_workspace (意识 2/4)")
        ...
        self.emotion_binder = EmotionCognitionBinder()
        self.logger.info(f"  ✅ working_memory (体验流, size={WORKING_MEMORY_SIZE})")
        self.logger.info(f"  ✅ predictive_coder (预测编码, tokens={PREDICTION_TOKENS})")
        self.logger.info(f"  ✅ emotion_binder (情感-认知绑定)")
    ...
    def aware_step(self, gpt2, tokenizer, prompt, hidden_now, hidden_prev,
        ...
        with torch.no_grad():
            ...
            # 用 hidden_prev 预测 hidden_now 对应的 token
            predicted_ids, top5_ids = self.predictive_coder.predict_next_tokens(gpt2, hidden_prev, PREDICTION_TOKENS)
            pred_result = self.predictive_coder.compute_prediction_error(
                predicted_ids, actual_first_token, top5_ids
            )
            ...
            logger.info(f"    🔮 predict: exact={pred_result['exact_correct']}, top5={pred_result['top5_correct']}, "
                       f"reward={pred_result['prediction_reward']:+.2f}")
            ...
            # === NEW 3: 情感-认知绑定 — valence 直接修改 hidden ===
        ...
        # Metacognition record outcome
        if self.metacognition is not None:
            try: self.metacognition.record_outcome(confidence, logprob_delta > 0, prompt[:50])
            except: pass

        # === NEW 1: 存入 working memory (跨周期体验) ===
        exp = Experience(
            cycle=cycle, timestamp=time.time(), topic=topic,
        ...
        # Hippocampus encode
        hippocampus.encode(hidden_now, topic, [], prompt, cycle,
                          logprob_baseline, logprob_stdp_after, confidence, self_confidence)
...
def load_latest_checkpoint(brain, hippocampus, curiosity, logger):
    ...
    try:
        ...
        logger.info(f"  ✅ resumed: c={prev_cycle} norm={state.get('stdp_norm',0):.4f} "
                    f"mems={len(hippocampus.memories)} wm={state.get('working_memory_summary',{}).get('size',0)}")
...
class GracefulExit:
    ...
    def _h(self, s, f):
        logging.getLogger("brain_v4").info(f"  [signal] {s}"); self.should_exit = True
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("🧠 brain_daemon_v4_aware starting")
    logger.info("  3 new mechanisms: working_memory + predictive_coding + emotion_binding")
    logger.info("=" * 80)
    ...
    if aicq.connected.is_set():
        ...
        aicq.send_message(f"🧠 brain v4 (aware) 上线\n3 个意识关键机制:\n"
                         f"1. 跨周期体验流 (working memory, {WORKING_MEMORY_SIZE} 个 cycle)\n"
                         f"2. 预测编码 (主动预测 + 误差 reward)\n"
                         f"3. 情感-认知绑定 (valence 直接修改 hidden)")
    ...
    while not graceful.should_exit:
        ...
        for from_id, content in aicq.get_incoming():
            ...
            wm_sum = brain.working_memory.get_summary()
            aicq.send_message(f"收到「{content}」\n当前: cycle {global_cycle}, "
                            f"confidence={brain.last_confidence:.3f}\n"
                            f"working memory: {wm_sum['size']} 体验, "
                            f"avg valence={wm_sum.get('avg_valence',0):+.3f}\n"
                            f"预测准确率: {brain.predictive_coder.get_accuracy():.2f}\n"
        ...
        try:
            ...
            cycle_conf = 0; cycle_recalled = 0; cycle_pred_correct = 0
            for i, prompt in enumerate(prompts):
                ...
                snap, mod_h, lp_b, lp_s, rb, cons = brain.aware_step(
                ...
                logger.info(f"     [{i+1}] lp_b={lp_b:+.3f} lp_s={lp_s:+.3f} Δ={lp_s-lp_b:+.4f} "
                           f"conf={cons.get('meta_confidence',0):.2f} "
                           f"pred={'✅' if cons.get('prediction_top5') else '❌'} "
                           f"wm={cons.get('wm_injection_norm',0):.3f} "
                           f"emo={cons.get('emotion_injection_norm',0):.3f} "
                           f"{'↩️' if rb else '✅'}")

            n = len(prompts)
            if n > 0:
                avg_lp_b = cycle_lp_base/n; avg_lp_s = cycle_lp_stdp/n
                avg_conf = cycle_conf/n
                pred_acc = cycle_pred_correct/n
                logger.info(f"  📊 avg: lp_Δ={avg_lp_s-avg_lp_b:+.4f}, rollbacks={cycle_rollbacks}/{n}, "
                           f"conf={avg_conf:.3f}, pred_top5={pred_acc:.2f}, recalled={cycle_recalled}")

            if brain.dmn is not None:
                dmn_inf = brain.dmn.get_cognitive_influence()
                if dmn_inf.get("is_active"):
                    logger.info(f"  💭 DMN active: idle={dmn_inf.get('idle_time',0):.0f}s")

            stdp_norm = float(brain.stdp_weight.norm().item())
            wm_sum = brain.working_memory.get_summary()
            reflection = (f"c{global_cycle}: 「{query}」 lp_Δ={avg_lp_s-avg_lp_b:+.4f} "
                         f"conf={avg_conf:.2f} pred={pred_acc:.2f} "
                         f"wm={wm_sum['size']} recalled={cycle_recalled} "
                         f"norm={stdp_norm:.4f}")
            ...
            if global_cycle % 5 == 0 and aicq.connected.is_set():
                aicq.send_message(
                    f"🧠 v4 cycle {global_cycle}\n"
                    f"学: {query}\n"
                    f"lp Δ={avg_lp_s-avg_lp_b:+.4f}\n"
                    f"置信度={avg_conf:.3f}\n"
                    f"预测top5准确率={pred_acc:.2f}\n"
                    f"working memory={wm_sum['size']} 体验\n"
                    f"  最近: {wm_sum.get('recent_topics',[])}\n"
                    f"  avg valence={wm_sum.get('avg_valence',0):+.3f}\n"
    ...
    wm_sum = brain.working_memory.get_summary()
    pred_acc = brain.predictive_coder.get_accuracy()
    logger.info(f"\n{'='*80}\n🧠 v4 session complete\n{'='*80}")
    ...
    logger.info(f"  working memory: {wm_sum['size']} experiences, avg_valence={wm_sum.get('avg_valence',0):+.3f}")
    logger.info(f"  prediction accuracy: {pred_acc:.2f}")
    logger.info(f"  consciousness: {'active' if brain.consciousness_state is not None else 'inactive'}")

    if aicq.connected.is_set():
        aicq.send_message(f"🧠 v4 session 结束\n"
                         f"cycles: {cycles_this_session}, time: {total_time/60:.1f}min\n"
                         f"lp improvement: {brain.total_logprob_improvement:+.4f}\n"
                         f"prediction accuracy: {pred_acc:.2f}\n"
                         f"working memory: {wm_sum['size']} experiences\n"
